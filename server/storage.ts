import {
  ...
  User as SelectUser // Add this import for the external user type
} from "@shared/schema";
...
export interface IStorage {
  ...
  findOrCreateUser(externalUser: SelectUser): Promise<User>; // Add method to interface
  ...
  // Add quizTitle to the signature
  createUserQuizResult(result: Omit<UserQuizResult, 'id' | 'completedAt' | 'grade'> & { completed: boolean }): Promise<UserQuizResult>;
  updateUserQuizResult(id: number, result: Partial<Omit<UserQuizResult, 'id' | 'userId' | 'quizId' | 'quizTitle' | 'grade'>> & { completed?: boolean }): Promise<UserQuizResult>; // Exclude quizTitle from update
...
export class DbStorage implements IStorage {
  ...
  // Add the new findOrCreateUser method here
  async findOrCreateUser(externalUser: SelectUser): Promise<User> {
    // Try to find the user by email first
    // Use externalUser.email which should exist based on SelectUser type
    let localUser = await this.getUserByEmail(externalUser.email);

    if (!localUser) {
      // If not found by email, create the user
      // Map external user data to local InsertUser schema
      // Ensure these fields match your actual schema definition in @shared/schema.ts
      const newUser: InsertUser = {
        // id: externalUser.id, // Remove this line - ID should be auto-generated by DB
        email: externalUser.email,
        firstname: externalUser.firstname ?? 'User', // Provide defaults if needed
        lastname: externalUser.lastname ?? '',
        password: 'EXTERNAL_AUTH', // Placeholder for externally authenticated users
        // Add other required fields from your local 'users' table schema if necessary
      };
      try {
        // createUser already validates using insertUserSchema
        localUser = await this.createUser(newUser);
        console.log(`Created local user for email: ${localUser.email} with ID: ${localUser.id}`);
      } catch (error) {
        // Handle potential race conditions (e.g., duplicate key error if created concurrently)
        console.error(`Error creating local user for email ${externalUser.email}:`, error);
        // Attempt to fetch again in case of race condition
        localUser = await this.getUserByEmail(externalUser.email);
        if (!localUser) {
          // If still fails, re-throw or handle as critical failure
          throw new Error(`Failed to find or create user after creation error for email: ${externalUser.email}`);
        }
         console.log(`Found local user for email ${externalUser.email} after handling creation error.`);
      }
    } else {
      // Optional: Update existing user data if necessary (e.g., name changes)
      // You might want to add logic here to update fields like firstname/lastname
      // if they have changed in the external source since the user was last seen.
      console.log(`Found existing local user for email: ${localUser.email} with ID: ${localUser.id}`);
    }

    // Ensure a user object is returned
    if (!localUser) {
        // This should theoretically not be reached if error handling above is correct
        throw new Error(`Could not find or create user for email: ${externalUser.email}`);
    }
  ...
  async createUserQuizResult(resultData: Omit<UserQuizResult, 'id' | 'completedAt' | 'grade'> & { completed: boolean }): Promise<UserQuizResult> {
    ...
    const dataToInsert = {
     ...
     // Validate before inserting (Schema already includes quizTitle)
   ...
   // Exclude quizTitle from the updatable fields
   async updateUserQuizResult(id: number, resultData: Partial<Omit<UserQuizResult, 'id' | 'userId' | 'quizId' | 'quizTitle' | 'grade'>> & { completed?: boolean }): Promise<UserQuizResult> {
    ...
    if (resultData.completed !== undefined) {
     ...
     delete (dataToUpdate as any).quizTitle; // Ensure quizTitle is not updated
